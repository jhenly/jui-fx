...
/**
 * Skin class for the {@code TopPromptTextField} control.
 *
...
public abstract class TopPromptTextFieldSkin extends SkinBase<TopPromptTextField> {
    ...
    ChangeListener<Boolean> changeInUsePromptText;
    ...
    /**
     * Specifies the prompt that is to be displayed over the textfield.
     *
    ...
    /**
     * Specifies the stub that is to be displayed under the textfield to the left of
     * the default text.
     *
    ...
    /**
     * Specifies the default text that is to be displayed under the textfield to the
     * right of the default stub.
     *
     * @return the default text property
     */
    public abstract ObjectProperty<Labeled> defaultTextProperty();


    private ObjectProperty<TextField> textFieldProperty = new SimpleObjectProperty<>(this, "textField");
    public final ObjectProperty<TextField> textFieldProperty() { return textFieldProperty; }
    public final TextField getTextField() { return textFieldProperty.get(); }
    private final void setTextField(TextField value) { textFieldProperty.set(value); }
    ...
    public TopPromptTextFieldSkin(final TopPromptTextField control) {
        ...
        // make sure to set textFieldProperty in constructor
    ...
    private void setUpListeners() {
        ...
        registerChangeListener(control.useDefaultTextProperty(), o -> useDefaultTextChanged());
...
//        registerChangeListener(textField.editableProperty(), e -> {});
        ...
        registerChangeListener(defaultTextProperty(), e -> updateChildren());
        
        // allows us to be notified of when tf's prompt should change
        changeInUsePromptText = (observer, old, ne) -> { handlePromptAsPromptText(ne); };
        control.usePromptAsPromptTextProperty().addListener(changeInUsePromptText);
    }
    ...
    private void updateChildren() {
        ...
        Labeled newDefaultText = defaultTextProperty().get();
        ...
        if (newDefaultText == null || !newDefaultText.equals(dtextNode)) {
            updateDefaultTextChild(newDefaultText);
        }
    ...
    private void handlePromptAsPromptText(boolean promptAsPromptText) {
        ...
        if (promptAsPromptText) {
            ...
            if (textFieldHasText.get()) {
                ...
                pseudoClassStateChanged(SETTLED_PSEUDO_CLASS, true);

                control.requestLayout();
                promptPane.setVisible(true);
            } else {
                pseudoClassStateChanged(SETTLED_PSEUDO_CLASS, false);
    ...
    private void updateDefaultStubChild(Labeled newDefaultStub) {
        ...
        if (newDefaultStub != null) {
            ...
            // set dstubNode to never grow horizontally so that default text will
            HBox.setHgrow(dstubNode, Priority.NEVER);

            /* be sure to get the control to request layout if promptNode's font changes,
             * since this will affect pref height and pref width */
            // dstubNode.fontProperty().addListener(fontInvalidated);
    ...
    private void updateDefaultTextChild(Labeled newDefaultText) {
    ...
    /* method that updates textfield text depending on control.useDefaultText */
    private void useDefaultTextChanged() {
        ...
        if (control.getUseDefaultText()) {
            // if TF is empty then fill it with default text
            if (!textFieldHasText.get()) {
    ...
    /**
     * layoutChildren helper method that handles promptPane's move and scale
     * transitions
     */
    private void
    ...
    @Override
    public void dispose() {
        // clean up usePromptAsPromptText change listener
        control.usePromptAsPromptTextProperty().removeListener(changeInUsePromptText);
